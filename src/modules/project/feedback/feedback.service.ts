...
import {
  ProjectFeedback,
  ProjectFeedbackDocument,
} from 'src/model/project-feedback/project-feedback.model';
import { CreateProjectFeedbackDto } from './dto/create-feedback.dto';

@Injectable()
export class FeedbackService {
  constructor(
    @InjectModel(ProjectFeedback.name)
    private feedbackModel: Model<ProjectFeedbackDocument>,
  ) {}

  async create(
    createFeedbackDto: CreateProjectFeedbackDto,
  ...
  async findByUserId(userId: string): Promise<ProjectFeedback[]> {
    return await this.feedbackModel.find({ userId }).populate('userId').exec();
  }
}
